...
test.describe('Quick Z-Index Test', () => {
  ...
  test('Quick z-index changes on 3 images and 3 texts', async ({ page }) => {
    const browserLogs = setupBrowserLogCapture(page);
    
    console.log('\n=== QUICK Z-INDEX TEST (Images + Texts) ===');
    
    // Setup: login, clean, create objects
    await setupTest(page);
    const imageIds = await createThreeImages(page);
    const textIds = await createThreeTexts(page);
    const allElementIds = [...imageIds, ...textIds];
    
    // Get initial state
    const initialState = await getZIndexState(page, allElementIds);
    logZIndexState(initialState, imageIds, 'Initial Images', 'Image');
    logZIndexState(initialState, textIds, 'Initial Texts', 'Text');
    ...
    for (let run = 1; run <= 2; run++) {
      ...
      // NEW: Cross-type conflict test - alternate between text and image
      ...
      for (let i = 0; i < Math.min(6, shuffledIds.length); i++) {
        ...
        console.log(`  ${type} ${index + 1} (${elementId.slice(-6)}): ${changeCount} changes`);
        
        await makeRandomZIndexChanges(page, elementId, index, changeCount);
        
        // Check for cross-type conflicts after each change
        const conflicts = await checkCrossTypeConflicts(page, imageIds, textIds);
        if (conflicts.length > 0) {
          allCrossTypeConflicts.push(...conflicts);
          conflicts.forEach(c => {
        ...
        await page.waitForTimeout(150); // Small delay between cross-type changes
      }
      ...
      for (let imgIndex = 0; imgIndex < imageIds.length; imgIndex++) {
        ...
        const { blinking, currentStates } = await checkForBlinking(page, allElementIds, previousStates);
      ...
      for (let txtIndex = 0; txtIndex < textIds.length; txtIndex++) {
        ...
        // Generate random number of changes (5-8)
        const changeCount = randomInt(5, 8);
        console.log(`\n--- Processing Text ${txtIndex + 1} ---`);
        
        await makeRandomZIndexChanges(page, elementId, txtIndex, changeCount);
      ...
      const runState = await getZIndexState(page, allElementIds);
      logZIndexState(runState, imageIds, `After Run ${run} - Images`, 'Image');
      logZIndexState(runState, textIds, `After Run ${run} - Texts`, 'Text');
    }
    
    // Final state
    const finalState = await getZIndexState(page, allElementIds);
    logZIndexState(finalState, imageIds, 'Final Images', 'Image');
    logZIndexState(finalState, textIds, 'Final Texts', 'Text');
    
    // Analyze logs to find culprit
    const analysis = analyzeLogsForCulprit(browserLogs, allElementIds);
    
    // Analyze duplicate z-index patterns
    const duplicateAnalysis = analyzeDuplicateZIndexPatterns(browserLogs, allElementIds, finalState);
    ...
    if (allBlinkingEvents.length > 0) {
      ...
      zIndexMismatches.forEach(b => {
        const type = b.id.startsWith('wbe-text-') ? 'Text' : 'Image';
        console.error(`❌ ${type} ${b.id.slice(-6)} Z-Index Mismatch: DOM=${b.domZIndex} Manager=${b.managerZIndex}`);
      });
    } else {
      console.log('\n✅ No blinking detected');
    }
    
    // Report cross-type conflicts
    const finalConflicts = await checkCrossTypeConflicts(page, imageIds, textIds);
    if (finalConflicts.length > 0 || allCrossTypeConflicts.length > 0) {
      const allConflicts = [...allCrossTypeConflicts, ...finalConflicts];
      console.error(`\n🚨 CROSS-TYPE CONFLICTS: ${allConflicts.length} conflicts detected`);
      const duplicates = allConflicts.filter(c => c.type === 'duplicate_zindex');
      const ordering = allConflicts.filter(c => c.type === 'ordering_mismatch');
      console.error(`  Duplicate z-indexes: ${duplicates.length}`);
      console.error(`  Ordering mismatches: ${ordering.length}`);
      duplicates.forEach(c => {
        console.error(`  ❌ Z-Index ${c.zIndex} shared by ${c.object1.type} ${c.object1.id.slice(-6)} and ${c.object2.type} ${c.object2.id.slice(-6)}`);
      });
      ordering.forEach(c => {
        console.error(`  ❌ ${c.object1.type} ${c.object1.id.slice(-6)} (Mgr:${c.object1.managerZ}, DOM:${c.object1.domZ}) vs ${c.object2.type} ${c.object2.id.slice(-6)} (Mgr:${c.object2.managerZ}, DOM:${c.object2.domZ})`);
      });
    } else {
      console.log('\n✅ No cross-type conflicts detected');
    }
    ...
    const relevantLogs = browserLogs.filter(log => 
      ...
      log.text.includes('[WB-E] debouncedFlush') ||
    ...
    for (const id of allElementIds) {
      ...
      if (!exists) {
        const type = id.startsWith('wbe-text-') ? 'Text' : 'Image';
        console.error(`❌ ${type} ${id.slice(-6)} no longer exists in DOM`);
      } else {
        ...
        if (finalState[id].domZIndex !== finalState[id].managerZIndex && finalState[id].domZIndex > 0) {
          const type = id.startsWith('wbe-text-') ? 'Text' : 'Image';
          console.error(`❌ ${type} ${id.slice(-6)} Z-Index Mismatch: DOM=${finalState[id].domZIndex} Manager=${finalState[id].managerZIndex}`);
        }
      }
    }
